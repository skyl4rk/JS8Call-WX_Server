...
# Convert from requested day to number of 3 hour weather periods as provided in API
    forecast_period = forecast_day * 8
...
# Calculate the maximum temperature in the next 24 hours
    max = forecast_json['list'][forecast_period]['main']['temp_max']
    forecast_step = forecast_period
    for forecast_step in range(forecast_step, forecast_step + 7):
        if  (forecast_json['list'][forecast_step]['main']['temp_max'] > max):
...
# Calculate the minimum temperature in the next 24 hours
    min = forecast_json['list'][forecast_step]['main']['temp_min']
    forecast_step = forecast_period
    for forecast_step in range(forecast_step, forecast_step + 7):
        if  (forecast_json['list'][forecast_step]['main']['temp_min'] < min):
            min = forecast_json['list'][forecast_step]['main']['temp_min']
...
# Maximum wind gust speed in next 24 hours
    ...
    forecast_step = forecast_period
    for forecast_step in range(forecast_step, forecast_step + 7):
        if  (forecast_json['list'][forecast_step]['wind']['gust'] > gust):
            gust = forecast_json['list'][forecast_step]['wind']['gust']
...
################
    ...
    # Please change the API_key to a key registered under your name at openweathermap.org:
    # Temporary API_key = "b8daf768cd03d5035a15728377db75f9"
