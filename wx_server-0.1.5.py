...
# Set display units
    ...
    if (display_unit_type == 'metric'):
        ...
        speed_unit = ' m/s'
# Print header
    tx_string = '\nWIND FORECAST FOR ' + forecast_gridsquare  + '\n'
...
def openweathermap_wx_api_call(forecast_gridsquare, forecast_day, user_API_key, display_unit_type, request_callsign):
...
# Check for weather trigger
                    if split_message[i] == wx_trigger or split_message[i] == wind_trigger:
...
# Run wx function to print and send wx report
                        if split_message[i] == wx_trigger:
                            openweathermap_wx_api_call(request_grid, request_day, API_key, unit_type, request_call)
