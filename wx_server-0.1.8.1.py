...
# Check for weather trigger
                    if split_message[i] == wx_trigger or split_message[i] == wind_trigger or split_message[i] == email_trigger:
                        if re.search('\d\\.\d', split_message[2]) and re.search('\d\\.\d', split_message[3]):
#                            print('re.search result: ')
                            try:
                                latitude = split_message[2]
                                longitude = split_message[3]
                                request_day = split_message[4]
                            except Exception as e:
                                print(e)
                        else:
                            try:
                                if re.search('[A-Za-z][A-Za-z][0-9][0-9]',split_message[2]):
                                    coords = mh.to_location(split_message[2])
                                    latitude = str(coords[0])
                                    longitude = str(coords[1])
                                    request_day = split_message[3]
                                else:
                                    coords = mh.to_location(default_grid)
                                    latitude = str(coords[0])
                                    longitude = str(coords[1])
